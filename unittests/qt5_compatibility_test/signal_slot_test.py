...
class Main(QObject):
    ...
    @slot(int, int)
    def simple_slot_method(self, offset: int, interval: int) -> None:
        @new_thread()
        def auto_update():
    ...
    def complex_slot_method(self, a: list, b: dict) -> list:
        print(a, b, ':l')
        return a + list(b.values())


app.register(Main(), 'main')  # 注意第一个参数是类的实例.
app.run(xpath('signal_slot_test.qml'))
